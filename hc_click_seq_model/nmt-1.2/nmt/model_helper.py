...
__all__ = [
    ...
    "fetch_last_nonzero", "output_hidden_state"
]
...
def compute_perplexity(model, sess, name):
  ...
  while True:
    try:
      loss, predict_count, batch_size,  _, _, _ = model.eval(sess)
      # print(total_loss, loss, total_predict_count)
      total_loss += loss
  ...
  eval_loss = total_loss / total_predict_count
  utils.print_time("%s  eval loss: %.6f" % (name, eval_loss),
                   start_time)
  return eval_loss 

def fetch_last_nonzero(state, len_info):
  [max_time, batch_size, num_units] = state.shape
  nonzero_state = []
  for i in range(batch_size):
    nonzero_state.append([])
  for i in range(batch_size):
    if len_info[i] <= max_time:
      for j in range(len_info[i]):
        if(np.count_nonzero(state[j][i]) != 0):
          nonzero_state[i].append(state[j][i])
  return np.vstack(np.array(nonzero_state))

def output_hidden_state(model, sess, name, output_file):
  """Compute perplexity of the output of the model.
  ...
  while True:
    try:
      _, _, _, decoder_logits, rebuild_logits, tgt_len = model.eval(sess)
      output = fetch_last_nonzero(decoder_logits, tgt_len)
      print(decoder_logits.shape)
      print(tgt_len)
      print(np.sum(tgt_len))
      np.savetxt(f_handle, np.array(output), '%.7f')
    except tf.errors.OutOfRangeError:
      break
