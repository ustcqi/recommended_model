...
def eval(sess, dssm, eval_part_list, eval_params):
  auc = 0.0  
  avg_loss = 0.0
  scores = np.zeros((1, 1))
  losses = np.zeros((1, 1))
  ...
  for part in eval_part_list:
    ...
    while True:
      try:
        ...
        pred_score, match_score, loss = sess.run([dssm.pred_score, dssm.match_score, dssm.loss], feed_dict={dssm.user_input : batch_user_feature, dssm.cross_input : batch_topic_feature, dssm.ad_input : batch_ad_feature, dssm.label : batch_label})
    ...
    for epoch in range(train_params["train_epoch"]):
      for part in train_part_list:
        ...
        while True:
          try:
            (batch_user_feature, batch_topic_feature, batch_ad_feature, batch_label) = sess.run([user_features, topic_features, ad_features, labels])
            sess.run(dssm.train_op, feed_dict={dssm.user_input : batch_user_feature, dssm.cross_input : batch_topic_feature, dssm.ad_input : batch_ad_feature, dssm.label : batch_label})
