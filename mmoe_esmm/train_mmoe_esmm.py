      ...
      for part in train_part_list:
        ...
        while True:
          try:
            ...
            loss, pred_score = sess.run([esmm.loss, esmm.pred_score], feed_dict={esmm.user_input : batch_user_input, esmm.cross_input : batch_cross_input, esmm.ad_input : batch_ad_input, esmm.hist_click_seq : batch_hist_click_seq, esmm.hist_click_length : batch_hist_click_length, esmm.target_ad : batch_target_ad, esmm.label : batch_label})
            # print(loss, pred_score)
            sess.run(esmm.train_op, feed_dict={esmm.user_input : batch_user_input, esmm.cross_input : batch_cross_input, esmm.ad_input : batch_ad_input, esmm.hist_click_seq : batch_hist_click_seq, esmm.hist_click_length : batch_hist_click_length, esmm.target_ad : batch_target_ad, esmm.ctr_label : batch_ctr_label, esmm.cvr_label : batch_cvr_label})
          except tf.errors.OutOfRangeError:
            break
        if i % 10 == 0:
          auc, avg_loss = eval(sess, esmm, eval_part_list, eval_params)
